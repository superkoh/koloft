...
const caller = (): { tabId: string; cwd: string } => ({ tabId: 't1', cwd: dir })
...
describe('koloft open / diff', () => {
  ...
  it('a file that is not there is refused before anything opens', async () => {
    const v = verbs()
    const reply = await v.open(['missing.md'], caller())
    expect(reply.ok).toBe(false)
    expect(reply.text).toContain(path.join(dir, 'missing.md'))
    expect(v.opened).toEqual([])
  })

  it('a file the Workbench cannot show is refused', async () => {
    const reply = await verbs({ canShow: false }).open(['plan.md'], caller())
    expect(reply.ok).toBe(false)
  })
})
...
describe('koloft note', () => {
  it('prints the note, and says so when it is empty', async () => {
    ...
    expect(await v.note([], caller())).toMatchObject({ ok: true, text: 'use npm ci\n' })
  })
  ...
  it('append puts the text on a line of its own at the end, joining unquoted words', async () => {
    ...
    const v = verbs()
    expect((await v.note(['append', 'run', 'the', 'tests'], caller())).ok).toBe(true)
  ...
  it('an unpinned workspace is refused', async () => {
    ...
    expect(reply).toMatchObject({ ok: false, text: NOT_PINNED })
  })

  it('an unknown word or an append with no text is a usage mistake, and the note is untouched', async () => {
    const v = verbs()
    expect(await v.note(['erase'], caller())).toMatchObject({ ok: false, exit: EXIT_USAGE })
    expect(await v.note(['append', ' '], caller())).toMatchObject({ ok: false, exit: EXIT_USAGE })
