...
/**
 * Scheduled jobs in a workspace — the EDGE black-box cases. The main-flow half
 * lives in `cron.spec.ts`; the two files share nothing on purpose, so either can move
 * without breaking the other. Every case id below is the one from
 * (retired with the PR that
 * shipped), and every string, class name and order
 * asserted here is the implementation contract's (§5 classes, §7 strings). A product
 * that says something else is a defect to report, never an assertion to soften.
 *
 * Two timing facts the whole file is built on:
 *  - a toast is taken down after 4 seconds, so a toast assertion comes BEFORE any
 *    slower wait (`edExpectToast` polls, so an earlier toast in the same run — the
 *    `started` one that precedes a failure — does not make it flake);
 *  - `Run now` is the driver everywhere except BB-E04, whose whole subject is the
 *    20-second tick finding a due that belongs to a time Koloft was not running.
 */
...
interface EdSeedJob {
  ...
  task?: string
  schedule: unknown
  /** defaults to the pinned ws-a — a job whose path is not pinned is dropped (BB-E13) */
  workspacePath?: string
  model?: string
  createdAt?: number
  enabled?: boolean
  /** written through as-is, newest first — a card that has already run before launch */
  history?: unknown[]
}
...
/** `<userData>/cron.json` exactly as it sits on disk — the cases' one observable for
 *  what was really saved (live runs are never in it). */
...
/** The hand-written store recipe: written BEFORE launch, since main loads it once. */
...
/** local `HH:MM`, the shape `<input type="time">` and a daily schedule both use */
function edHHMM(t: Date): string {
  return `${edPad(t.getHours())}:${edPad(t.getMinutes())}`
}

/** `yymmdd-HHMM` in local time — the stamp a run folder's name carries (§4.4). */
...
// ---- launching and driving the dialog ---------------------------------------------
...
/** The one door: the workspace row's menu (right-click — the §0 menu rule). */
...
/** the `.fcol` that belongs to one form row — the grid pairs them as siblings */
...
/** every complaint the form is showing right now, as exact strings */
...
/** Make a job the way a person does — through the form, defaults everywhere else. */
...
/** A toast assertion that rides out an earlier toast in the same run (a failure is
 *  always preceded by `started` when the launch itself got off the ground). */
...
/** every `.hist-state` line, newest first (a live "done" run sits above the written
 *  history while it lasts — §7.4). `.hist-wt` is a SIBLING of `.hist-state`, so these
 *  are the state sentences alone; a case that names the worktree reads that span. */
...
/** the `-w` value the newest launch carried, or undefined for an in-place run */
...
test('BB-E01: a due while the last run is still open is skipped, and skips fold into one line', async ({
  ...
  try {
    ...
    // the run's first turn ends on its own (the fixture's Stop hook), which is what
    // makes the next due an overlap rather than a fresh start
    await expect
    ...
    // second due: skipped, said out loud, and written down once. The live "done" run
    // keeps the top of the list (§7.4), so the written line is the one below it — and
    // it carries no worktree, unlike the live row above.
    await edRunNow(page)
    ...
    // third due: the same line counts up rather than growing a second row, and the
    // press is answered again — a button that says nothing back reads as broken. (The
    // silence is for dues the CLOCK folds, which nobody asked for; that half is a unit
    // test, cronRunner.test.ts, since e2e cannot wait out a 20-second tick per fold.)
    ...
    expect(await edHistRows(page).count()).toBe(2) // still ONE skip line
    // and that one row now covers a stretch of time, not just its first minute
...
test('BB-E02: a workspace that is not a git repo runs in place, and the form says so', async ({
  ...
  test.setTimeout(180_000)
  gitInit(env.workspaces.a) // the global precondition; ws-b stays a plain folder
  const { app, page } = await edLaunch(env)
  try {
...
// ==================================================================================
// BB-E03 (automated half — the unit half owns worktreeNameFor's own rules)
// ==================================================================================
...
test('BB-E03: two runs never share a worktree, even when only the branch is left', async ({
  ...
  test.setTimeout(180_000)
  gitInit(env.workspaces.a)
  // Leave the FOLDER free and the BRANCH taken, for this minute and the next three:
  // the run's folder name is stamped with the minute it LAUNCHES in, and a cold app
  // launch plus a form fill can cross a minute boundary (or three, on a slow machine
  // near :59). Covering the window is what keeps the `-2` below from missing it.
  const now = new Date()
  for (let i = 0; i <= 3; i++) {
  ...
  try {
    ...
    await expect
      .poll(() => fs.existsSync(path.join(env.workspaces.a, '.claude', 'worktrees', name ?? '')), {
        timeout: 30_000
      })
      .toBe(true)
  } finally {
...
test('BB-E04: a time that passed while Koloft was not running is a miss, not a catch-up', async ({
  ...
  try {
    ...
    // the job's OWN time, not any time: a miss must not move the clock the card
    // promises next, and `\d\d:\d\d` would pass on a recomputed wrong one
...
test('BB-E05: the workspace folder is missing at fire time', async ({ env }) => {
  ...
  try {
    ...
    // the folder goes while the dialog is up — the menu item would be gone (BB-E32),
    // so this is the only way in to the button
...
// What carries this case is the marker SURVIVING, not merely appearing: measured
//, an earlier fixture wrote its canned startup records in the same
// millisecond as the `/need-approval` Notification, and the tracker's
// `resumeWorkingIfStale` then resumed 'working' and cleared the marker ~400 ms
// after it was raised. So the 10-second wait below is the assertion, and a version
// of this test that polls for the marker instead of waiting would pass on a run
// nobody could have acted on.
test('BB-E06: a permission prompt during a run alerts you and nothing closes', async ({ env }) => {
  test.setTimeout(180_000)
  gitInit(env.workspaces.a)
  const { app, page } = await edLaunch(env)
  try {
    ...
    // the alert reached main's pending set, for the tab this run really owns. This
    // run is the only session open, so its pty id is the only mounted one — equality
    // here says the marker names THE RUN, where `toContain` would pass on any tab.
    ...
    // nothing was closed and nothing was called finished
    await expect(wsRows(page, 'ws-a')).toHaveCount(1)
...
test('BB-E07: claude dies before it ever binds', async ({ env }) => {
  ...
  try {
    ...
    expect(readCalls(env)).toHaveLength(1) // the launch DID happen

    // the job is not blocked by the failure
...
test('BB-E08: a run that never starts is stopped at the deadline', async ({ env }) => {
  ...
  try {
    ...
    // `notifyPlain` (src/main/index.ts) must never CONSTRUCT a Notification under
    // KOLOFT_TEST_BACKGROUND (rule D8). A deadline kill is its only caller, so this
    // line is the whole suite's one reading of that path.
    expect(
      await app.evaluate(
        () => (globalThis as { __koloftOsNotifCount?: number }).__koloftOsNotifCount ?? 0
      )
    ).toBe(0)
    // the fixture HOME has no `~/.claude.json`, so as far as Koloft can tell claude has
    // never been opened in ws-a — and that is the one thing worth saying about a run
    // that never started (BB-E35 owns the form's half of the same fact)
    await expect
      .poll(async () => (await edHistStates(page))[0] ?? '', { timeout: 15_000 })
      .toBe(
    ...
    // the tab it was quietly given is gone, and so is the process behind it
    ...
    // and the job still runs afterwards
    fs.rmSync(hangFile)
    await edRunNow(page)
    await waitForCalls(env, 2)
    await expect(edCronRows(page)).toHaveCount(1, { timeout: 60_000 })
...
// ==================================================================================
// BB-E10 (both halves — one launch each, since removing the workspace deletes its jobs)
// ==================================================================================
...
test('BB-E12: a renderer reload mid-run brings the run back', async ({ env }) => {
  ...
  const { app, page } = await edLaunch(env)
  try {
    // a session of the person's own, so "the active tab did not change" has a subject
    ...
    // the run is still marked as one, in the same folder…
    ...
    // …and the person is still on their own session
...
test('BB-E13: broken entries in the store are dropped, the rest load; a bad model is reported', async ({
  ...
  gitInit(env.workspaces.a)
  edWriteStore(env, [
    { id: 'good-1', name: 'Alpha job', schedule: { kind: 'daily', at: '21:00' }, createdAt: 1000 },
    {
      id: 'bad-clock',
      name: 'Bad clock',
  ...
  try {
    ...
    // a loader that complained out loud would toast on boot, so watch a window
    // rather than take one sample: a 4 s toast outlives an instant check but not
    // this, and `toHaveCount(0)` would wait for it to dismiss and then pass
    for (let i = 0; i < 8; i++) {
      expect(await page.locator('.toast').count()).toBe(0)
      await page.waitForTimeout(200)
    }
    // the dialog is the ONLY thing open, and it is the jobs dialog
    await expect(page.locator('.modal')).toHaveCount(1)
    await expect(page.locator('.modal')).toHaveClass(/\bcronjobs\b/)

    // saving any job rewrites the file — the two dropped entries do not come back
...
// ==================================================================================
// BB-E17 (automated half — the words themselves are the unit half)
// ==================================================================================
...
test('BB-E17: Pick days with no day selected refuses the save', async ({ env }) => {
  ...
  try {
    ...
    await expect.poll(() => edErrors(page), { timeout: 10_000 }).toContain('Pick at least one day.')
...
test('BB-E18: the form refuses what the launch could not carry', async ({ env }) => {
  ...
  try {
    ...
    // a task the launch line would read as flags
    await edField(page, 'Name').fill(JOB)
    ...
    // a model name the launch line could not carry
    await edField(page, 'Name').fill(JOB)
    ...
    await expect.poll(() => edErrors(page), { timeout: 10_000 }).toContain('Use a time like 09:00.')
    await expect(save).toBeDisabled()
  } finally {
    await app.close().catch(() => {})
  }
...
// ==================================================================================
// BB-E25 (both halves — the hint is read from settings at launch)
// ==================================================================================
...
test('BB-E32: the menu item is hidden for a missing workspace, and the job survives', async ({
  ...
  try {
    const head = page.locator('.ws-head', { hasText: 'ws-a' })
    await expect(head).toBeVisible({ timeout: 30_000 })
    // the ⏰ forecast row is up while the folder is there
    const nextRow = page
      .locator('.ws')
    ...
    // the folder goes; the `missing` flag is computed on a rescan, and adding another
    // workspace is the suite's established way to force one (T-WS-04)
    fs.renameSync(env.workspaces.a, `${env.workspaces.a}-gone`)
    ...
    // nothing can run in a folder that is gone, so the row goes with the door
    ...
    // and back again: the job was never deleted
...
test('BB-E33: a folded miss names both its ends, and the card reads the newest one', async ({
  ...
  // The nap: 85 dues missed between two moments. Both moments must land on ONE
  // calendar day, or the words would be "yesterday …" for the older end — so between
  // midnight and 02:00 the nap is anchored to a fixed midday instead of to "2 h ago".
  const now = new Date()
  ...
  // half an hour out: well clear of the boot tick's own five-minute miss window, so
  // nothing gets written above the seeded row
  const at = edHHMM(new Date(now.getTime() + 30 * 60_000))
  edWriteStore(env, [
    {
      name: JOB,
  ...
  try {
    ...
    // the fold is counted out loud, and the row spans the whole nap rather than
    // reading as one miss at the minute it began
    ...
    // amber belongs to a skip and red to a failure; a miss is the plain full stop
    await expect(edHistRows(page).locator('.hist-state > span')).toHaveClass('dot')

    // THE assertion this case is for: "last run" is the NEWEST end of the folded row.
    // Were the card to read `dueAt`, it would name the minute the nap BEGAN — and
    // every clause above would still pass.
...
/** One dirty buffer — the smallest copy of file-edit-guards' `dirtyEditor`: open
 *  `config/app.json` in the panel's editor and type into it. */
...
// BB-E34 (an id minted here, not in the former cases file — the route was found while
// wiring the jobs sentence) — the THIRD way out of `removePlan`: nothing running, but
// unsaved files. It
// raises the plain unsaved question (UnsavedDialog) rather than the confirm BB-E10 drives,
// and it is the one route that never asks main — so the count in its sentence is the
// RENDERER's own (`WorkspaceSidebar.removeWorkspace` → `setUnsavedPrompt({ jobs })`).
// unsavedGuard.test.ts pins the sentence; only this case pins that the number reaches it.
//
// "Nothing running but dirty" needs a live conversation tab whose row is NOT running, and
// ⇧⌘R's resume window is where that shape exists: the old pty is killed (the row turns
// cold, and a resume is not a launch, so no pending row stands in for it) while the tab
// lives on carrying both the session anchor and the edit buffers (store.ts `rekeyOwner`).
// `fake-claude-delay` holds the re-bind off long enough to click through it. `st-pending`
// is the barrier to wait on: the sidebar paints it exactly when a row is neither running
// nor pending and a live tab still claims it — the state `removeWorkspace` counts as
// running = 0.
test('BB-E34: removing a workspace mid-resume asks about the unsaved file and names the job', async ({
  env
}) => {
  test.setTimeout(240_000)
  ...
  try {
    ...
    // read at process start, so it only ever reaches the claude the restart spawns
    fs.writeFileSync(path.join(env.home, 'fake-claude-delay'), '60000')
    await app.evaluate(({ Menu }) => {
      Menu.getApplicationMenu()?.getMenuItemById('restart-session')?.click()
    })
    await waitForCalls(env, 2) // the replacement claude is up, and will not bind for a minute
    ...
    // the files half, which is the other route's business (file-edit-guards BB-C24)…
    await expect(hint).toContainText('Unsaved changes in config/app.json.')
    // …and the half this case carries: the jobs the same click would delete
    await expect(hint).toContainText('1 scheduled job will be deleted. Jobs do not come back.')

    // Cancel: the removal never ran, so the job is still on disk
    ...
    // the quit guard would raise this very question and hold the teardown open for good
    await closeDiscardingEdits(app).catch(() => {})
  }
})

// ==================================================================================
// BB-E35 (an id minted here, not in the former cases file)
// ==================================================================================

// The trust question is invisible to Koloft: claude asks it BEFORE SessionStart and
// `--dangerously-skip-permissions` does not skip it (contract §9), so a job in a folder
// nobody has ever opened claude in dies at the 90 s deadline every single time, saying
// only "Claude did not start". The form is where that is cheap to prevent — one hint,
// and Save stays allowed, because the fix is a session the person starts by hand.
//
// The two halves are one case on purpose: the hint has to be driven by the FILE, and a
// warning that is simply always on would pass the first half by itself.
test('BB-E35: the form warns when claude has never been opened in the folder, and stops warning once it has', async ({
  ...
  expect(fs.existsSync(claudeJson)).toBe(false) // the fixture HOME has never run claude
  ...
  try {
    ...
    // a hint, not a refusal: nothing is red, and the job still saves
  ...
  // the person answers claude's question in a real session, so claude writes the folder
  // into ~/.claude.json (the key names come off the 2.1.263 binary — cron-real-smoke's
  // `seedClaudeConfig` seeds the same shape)
  fs.writeFileSync(
    claudeJson,
  ...
  try {
    ...
    // the row itself is still there — the hint is what went away, not the note above it
