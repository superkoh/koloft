...
/**
 * The "never forced to leave the app" capability set, now the Workbench's `web` kind
 * (FR-37 — the merge changes none of it; case ids are the original
 * case list's): login (UA + OAuth popup chain + cookie persistence),
 * downloads, certificates, upload, page permissions, clipboard, HTTP basic auth.
 *
 * BB-M18, BB-M19, BB-M20, BB-M23, BB-M24, BB-M26, BB-N08, BB-C22, BB-C62, BB-C63,
 * BB-C65, BB-C69, BB-C70.
 *
 * Nothing imports from src/ — each oracle is a fixture-server request log, an
 * injected-seam file (KOLOFT_DOWNLOAD_DIR / openCalls / externalOpens), or the DOM.
 */

/**
 * The Given every case shares: the panel showing, with a blank `web` tab up.
 *
 ...
 * so a `web` tab has to be minted through the ＋'s two-item dropdown before any url can
 * be typed.
 */
...
/**
 * How many proceed affordances are on screen right now, host surface and guests alike —
 * the PRD fixes the label, not whether the interstitial is painted by Koloft or by the
 * guest, so the oracle counts both places.
 */
...
// BB-M26 — §05D-1 / SEC-12: the guest self-describes as a plain Chromium browser. The
// echo server's recorded User-Agent is the oracle; whether Koloft's OWN outbound identity
// stayed put is BB-N08's half, not asserted here.
...
// BB-N08 — SEC-12: the rewrite is a partition setting, not a new identity for Koloft. The
// guest half is measured on the same echo server as BB-M26; Koloft's own half is
// `app.userAgentFallback`, the string every Koloft-side request (update check, account
// probe) inherits when nothing overrides it.
test('BB-N08: the guest UA change is confined to the partition and does not alter Koloft’s own identity', async ({
  ...
  try {
    ...
    // the guest speaks as a plain Chromium…
    expect(server.requestsFor('/echo')[0].userAgent).not.toMatch(/Electron\//i)
    // …while Koloft's own outbound identity is untouched
...
// BB-M24 — D12 / §05D-1 / §05D-8: a login popup is a tab, never an OS window. Driven
// against the fake IdP so the popup has to walk the real OAuth shape (/login → 302
// /callback?code= → 302 /app) instead of a single hop.
test('BB-M24: `window.open` / OAuth popup is routed into a new Browser tab, never a system window', async ({
  ...
  try {
    ...
    // the popup became a second Browser tab that finished the redirect chain
    ...
    // …and nothing left Koloft: no extra OS window, no `open`
...
// BB-M23 — D11 / §06F: `persist:koloft-browser` keeps the login cookie across a restart.
// Only the `env` fixture is used: this spec owns two launches against the same home.
...
// BB-M18 — §05D-2 / TEST-4: a download response lands on disk instead of being thrown at
// the OS. KOLOFT_DOWNLOAD_DIR is the injected destination, so the developer's real
// ~/Downloads is never a party to this.
test('BB-M18: a download response lands a file on disk with a toast', async ({
  app,
  page,
  env
}) => {
  ...
  try {
    ...
    // toast wording is non-contractual per PRD §06E; the reveal affordance is what's pinned
    ...
    // B9 (replaces F3): the download list is the door to a finished file
    // now, so the toast is a notice again — it goes on its own. Both ends are asserted:
    // still up early (the user gets a chance at it), gone after the window.
    await page.waitForTimeout(2000)
    await expect(page.locator('.toast')).toContainText('Reveal in Finder')
    await page.waitForTimeout(4000)
    await expect(page.locator('.toast')).toHaveCount(0)
    // and the file is still reachable, which is what makes the auto-dismiss safe
...
// BB-C69 — §05D-2 / G0-3: the download belongs to the person, not to the guest that
// started it. ⌘W lands while the response is provably still trickling.
test('BB-C69: an in-flight download survives closing the tab that started it', async ({
  ...
  try {
    ...
    // in flight: the server has the request open and has not written its last byte yet
    ...
    // the initiating guest is really gone before the download can finish
    await expect
      .poll(() => guestPages(app).filter((p) => p.url().includes('/link')).length, {
        timeout: 30_000
      })
      .toBe(0)
    // …and "before it can finish" is measured, not hoped for: the last byte is still
    // trickling, so a download that lands later can only have survived the close
...
// BB-M19 — §05D-4 / B6: a self-signed https host gets Chrome's shape — interstitial,
// "Proceed (unsafe)", and a per-host memory for the rest of the session. The alias must
// be mapped onto the loopback BEFORE the launch, so this spec launches by hand.
test('BB-M19: a certificate error shows a Chromium interstitial with a working "continue" and per-host memory', async ({
  ...
  try {
    ...
    try {
      ...
      // the interstitial stands between the user and the page: nothing was fetched yet
      ...
      // a later navigation to the SAME host proceeds on its own (per-host exception)
...
// BB-C22 — SEC-8 / B6 / D14: the same self-signed server, spelled `localhost`. There is
// no local-is-trusted shortcut: the interstitial stands in exactly as it does for the
// non-local alias in BB-M19, and the page is fetched only after the user continues.
test('BB-C22: certificate handling is not special-cased for localhost', async ({ env }) => {
  ...
  try {
    ...
    try {
      ...
      // no auto-accept: the interstitial is up and nothing has been fetched
      await expect.poll(() => proceedCount(app, page), { timeout: 40_000 }).toBeGreaterThan(0)
...
// BB-C65 — SEC-8: the exception is keyed by HOST, not by certificate. Both aliases are
// the same self-signed server, so only a host-keyed exception can tell them apart.
test('BB-C65: a certificate exception is scoped to the host and does not leak to another host on the same certificate', async ({
  ...
  try {
    ...
    try {
      ...
      // the user has already continued on aliasA
      await typeInAddressBar(page, tls.aliasUrl('koloft-a.test', '/a'))
      await clickProceed(app, page)
      await expect.poll(() => tls.count('/a'), { timeout: 40_000 }).toBe(1)

      // aliasB, same certificate: the interstitial is back and the page is not fetched
      await typeInAddressBar(page, tls.aliasUrl('koloft-b.test', '/b'))
      await expect.poll(() => proceedCount(app, page), { timeout: 40_000 }).toBeGreaterThan(0)
      expect(tls.count('/b')).toBe(0)

      // …while aliasA still goes straight through
...
// BB-M20 — §05D-3 / TEST-5: the upload chooser reaches the page, so a form can be
// finished without a detour through Safari.
...
/** One user click that asks for every P1-denied capability and reports each outcome. */
...
// BB-C63 — SEC-3 / §05D-6. P1 closed Electron's auto-approve hole by denying every
// permission outright; a later change (B7) replaced that with a prompt for the five the
// user can judge. What still has to hold — and what this case now pins — is the half
// that was never about the prompt: an UNANSWERED request grants nothing, and
// openExternal never becomes a permission at all. The prompt's own behaviour is
// bb-127-closeout.spec.ts's; this stays the "nothing leaks by default" guard.
test('BB-C63: an unanswered permission request grants nothing, and openExternal stays out', async ({
  ...
  try {
    ...
    // B7: the four promptable families now raise a bar and WAIT — so the probe's own
    // results stay pending, which is the correct answer to "was anything granted?".
    // Deny them all and the page must land exactly where P1 left it.
    // Each family raises its bar in turn, and a bar takes a moment to arrive — under
    // load, longer than the fixed pause this loop used to take between clicks. Checking
    // `count() > 0` before the first bar had rendered ended the loop with nothing
    // denied, every request left pending, and the result below empty for the full 40s
    // (measured: the suite's most frequent hard failure). Wait for each bar instead, and
    // stop only when none follows.
    ...
    for (let i = 0; i < 6; i++) {
      try {
        ...
        break // no further ask: every family has been answered
      }
      await deny.click()
    }
    ...
    expect(outcome.geolocation).toBe('rejected:1') // PERMISSION_DENIED

    // openExternal is a permission too: the page's own navigation must not reach the OS
...
// BB-C70 — §08 P2 acceptance / TEST-9: the page's "Copy" button works, and the proof is
// the guest-side promise plus the absence of any main-side clipboard read. Reading the
// real clipboard back as the oracle is banned — it is the developer's clipboard.
test('BB-C70: a page "Copy" button\'s writeText() resolves and the grant is recorded, with the real clipboard untouched', async ({
  ...
  try {
    ...
    // count main-side reads at the Electron boundary; the return value proves the
    // counter is actually wired in, so a later zero means "never read", not "never armed"
    ...
    // resolved = the permission handler granted THIS request (a denial rejects it)
    await expect(guest.locator('#result')).toHaveText('resolved', { timeout: 30_000 })

    const reads = await app.evaluate(
      () =>
        (globalThis as unknown as { __koloftClipboardReads?: number }).__koloftClipboardReads ?? -1
    )
    expect(reads).toBe(0)
  } finally {
    await server.close()
...
// BB-C62 — §05D-5 / SEC-10: basic auth challenges only the main frame, in a DOM modal
// that states origin + realm and pre-fills nothing. The sub-resource half runs FIRST and
// no credentials are ever submitted, so Chromium's own per-session credential cache
// cannot color either result.
test('BB-C62: HTTP basic auth challenges only the main frame and shows origin+realm without memory', async ({
  app,
  page,
  env
}) => {
  ...
  try {
    ...
    // a 401 sub-resource inside a 200 main frame: no credential prompt (anti-phishing)
    ...
    // a main-frame 401 does prompt — in the DOM, never a native dialog
