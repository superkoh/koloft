...
// Exercise the real CLI chooser, process transport, persisted history and UI lifecycle.
// The fake models the external CLI protocol; no session is injected into renderer state.
...
/** The method icon is drawn only where both kinds share a workspace (D2), so a ws-a with
 *  Codex sessions alone carries no icons at all. What holds either way: a Codex row never
 *  wears the Claude icon. */
function codexRows(page: Page): Locator {
  return wsRows(page, 'ws-a').filter({
    hasNot: page.getByRole('img', { name: 'Claude', exact: true })
  })
}
/** The default method's own menu item. With Codex installed both methods are usable, so
 *  the item names the current default; `method: null` is the one-usable case, where it
 *  goes back to plain "New session". */
...
test('two saved Codex sessions remain switchable through native startup prompts and a new session', async ({
  ...
  installCodex(env)
  // switching between saved rows is the subject here; a contextual card over the
  // sidebar would be in the way (the worktree hint has its own case below)
  ...
  try {
    ...
    // Human confirmation can take longer than the old transport's 15-second deadline.
...
test('the workspace entrance starts the default method straight away', async ({ env }) => {
  ...
  try {
    const page = await app.firstWindow()
    await waitBooted(page)
    // both methods are usable here, so both per-workspace surfaces name the second one
    await expect(
    ...
    // …and with two of them the default item names itself too, still launching on click
    ...
    // by the time the session launched, any dialog the click might raise would be up
    await expect(page.locator('.modal')).toHaveCount(0)
    expect(codexCalls(env)).toHaveLength(0)
    // and with only Claude sessions in it, the workspace says nothing about methods (D2)
    await expect(
...
test('the second method has its own entrance and does not become the default', async ({ env }) => {
  installCodex(env)
  const app = await launchApp(env)
  try {
    const page = await app.firstWindow()
    ...
    // the saved default is untouched: the plain entrance still starts Claude
    await newIn(page)
    await expect.poll(() => readCalls(env).length).toBe(1)
...
test('with no claude on this Mac the plain entrance starts Codex', async ({ env }) => {
  ...
  try {
    ...
    await waitBooted(page)
    const menu = await openMenu(page, page.locator('.ws-head', { hasText: 'ws-a' }))
    // one usable method means one entrance — the saved default is Claude, but Claude is
    // not on this Mac, so the plain entrance is the Codex one
...
test('the worktree dialog still asks, and its buttons name both methods', async ({ env }) => {
  ...
  try {
    ...
    // a dialog that opens has not launched anything, and ⏎ without a name still does not
    await page.keyboard.press('Enter')
    ...
    // the second button wears the key that starts it, the way the primary wears ⏎
...
test('⌘N ⏎ launches while the Codex probe is still out', async ({ env }) => {
  installCodex(env)
  // the fake takes ten seconds to answer `--version`; the four-second poll below is the
  // assertion — a launch that waited on the probe could not land inside it
  fs.writeFileSync(path.join(env.home, 'fake-codex-version-delay'), '10000')
  const app = await launchApp(env)
  try {
    const page = await app.firstWindow()
    await waitBooted(page)
    ...
    // no second button yet = the probe has not answered, so the poll below is timing
    // the launch against a probe that is genuinely still out
    await expect(
      pickerDialog(page).locator('.modal-foot button[data-default="false"]')
    ).toHaveCount(0)
    await page.keyboard.press('Enter')
    await expect.poll(() => readCalls(env).length, { timeout: 4000 }).toBe(1)
    expect(codexCalls(env)).toHaveLength(0)
  } finally {
    await quitAndClose(app)
  }
...
test('in the picker ⇧⏎ starts the other method', async ({ env }) => {
  ...
  try {
    ...
    // the list has no second entrance to click, so this key is the only way a keyboard
    // reaches the other method
...
test('Codex and Claude coexist; Codex closes, resumes and restarts without Workbench', async ({
  ...
  try {
    ...
    // D2: the icon reads as a contrast, so it is drawn exactly where both kinds meet
    await expect(
      wsRows(page, 'ws-a').getByRole('img', { name: 'Claude', exact: true })
    ).toHaveCount(1)
    await expect(wsRows(page, 'ws-a').getByRole('img', { name: 'Codex', exact: true })).toHaveCount(
      1
    )
    ...
    const statusOverride = (argv: string[]): string | undefined =>
      argv.find((value, index) => value.startsWith('tui.status_line=') && argv[index - 1] === '-c')
    const preset = statusOverride(first.argv)
    expect(preset).toBeDefined()
    expect(JSON.parse(preset!.slice('tui.status_line='.length))).toEqual(
      expect.arrayContaining(['context-used', 'git-branch', 'model-with-reasoning', 'current-dir'])
    )
...
test('mixed sessions receive worktree and Codex approval hints without Claude-only guidance', async ({
  ...
  try {
    ...
    await codexRows(page).click()
    await expect(page.locator('.term-island .term-wrap:visible')).toContainText('Approve command?')
...
test('Codex recovers a worktree preparation interrupted before any session bound', async ({
  ...
  try {
    ...
    // only Codex can recover, so Cancel and one button is the whole footer — a greyed
    // Claude button would still be saying there is a room behind that door
    ...
    // a row click starts it too, whatever the default method is
...
test('the saved default method is what an entrance starts, across a restart and a switch-off', async ({
  env
}) => {
  installCodex(env)
  gitInit(env.workspaces.a)
  ...
  try {
    ...
    // Accounts is about Claude's account pool; it only says where Codex signs in
    ...
    // the entrance names and starts the saved default with nothing in between
    ...
    page = await app.firstWindow()
    await waitBooted(page)
    // the worktree dialog is the one surface that still asks, and it asks default-first
    await newIn(page, true)
    ...
    // one method again: the entrance starts Claude, and the second one is off the menu
    const menu = await openMenu(page, page.locator('.ws-head', { hasText: 'ws-a' }))
    await expect(menu.locator('.mi', { hasText: 'New Codex session' })).toHaveCount(0)
    await menu.locator('.mi', { hasText: 'New session' }).click()
    await expect.poll(() => readCalls(env).length).toBe(1)
    await expect(page.locator('.modal')).toHaveCount(0)
    expect(codexCalls(env)).toHaveLength(2)
    // The creation boundary also refuses a direct request; resume/restart keep working.
