...
test.describe('Show ignored files: off hides every ignored entry; on shows everything and marks the ignored ones, directories included', () => {
  function rowInSection(page: Page, abs: string, section = 'tree'): Locator {
  ...
  async function browseReady(page: Page, root: string): Promise<void> {
    ...
    await expect(rowInSection(page, root)).toBeVisible({ timeout: 30_000 })
    await expect(rowInSection(page, `${root}/config`)).toBeVisible({ timeout: 30_000 })
  }
  ...
  test('BB-M01 (A-01, A-02, A-03, A-05): the Show-ignored switch reveals every ignored entry, directories included and marked, and both on and off survive a restart', async ({
    ...
    await expect(rowInSection(page, ed.env)).toHaveCount(0)
    await expect(rowInSection(page, ed.venvDir)).toHaveCount(0)

    await chip(page).click()
    await expect(chip(page)).toHaveClass(/\bon\b/)
    await expect(rowInSection(page, ed.env)).toBeVisible({ timeout: 20_000 })
    await expect(rowInSection(page, ed.env)).toHaveClass(/\bignored\b/)
    await expect(rowInSection(page, ed.env)).toHaveAttribute('title', EDIT.ignoredTitle)

    for (const dir of [ed.venvDir, fx.paths.ignoredDir]) {
      await expect(rowInSection(page, dir)).toBeVisible({ timeout: 20_000 })
      await expect(rowInSection(page, dir)).toHaveClass(/\bignored\b/)
      await expect(rowInSection(page, dir)).toHaveAttribute('title', EDIT.ignoredTitle)
    }
    await expect(rowInSection(page, fx.paths.heavyDir)).toBeVisible({ timeout: 20_000 })
    await expect(rowInSection(page, fx.paths.heavyDir)).toHaveClass(/\bignored\b/)
    await expect(rowInSection(page, fx.paths.heavyDir)).toHaveAttribute(
      'title',
      EDIT.hiddenByDefaultTitle
    )
    await expect(rowInSection(page, ed.venvFile)).toHaveCount(0)
    await rowInSection(page, ed.venvDir).click()
    await expect(rowInSection(page, ed.venvFile)).toBeVisible({ timeout: 20_000 })
    await expect(rowInSection(page, ed.venvFile)).toHaveClass(/\bignored\b/)
    await rowInSection(page, ed.venvDir).click()
    await expect(rowInSection(page, `${fx.root}/docs`)).toBeVisible()
    ...
    await expect(rowInSection(restarted.page, ed.env)).toBeVisible({ timeout: 25_000 })

    await chip(restarted.page).click()
    await expect(rowInSection(restarted.page, ed.env)).toHaveCount(0, { timeout: 20_000 })
    await expect(rowInSection(restarted.page, ed.venvDir)).toHaveCount(0)
    await expect(rowInSection(restarted.page, fx.paths.heavyDir)).toHaveCount(0)
    ...
    await expect(rowInSection(again.page, ed.env)).toHaveCount(0)
  ...
  test('BB-M03 (A-06): a file row can be handed to the system default app, recorded at the leaveForOS choke point', async ({
    ...
    await rowInSection(page, `${fx.root}/config`).click()
    await expect(rowInSection(page, `${fx.root}/config/app.json`)).toBeVisible({ timeout: 20_000 })
    await rowInSection(page, `${fx.root}/config/app.json`).click({ button: 'right' })
