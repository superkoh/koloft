...
test.describe('browser extensions (black-box cases; BB-M12 needs the real Web Store and BB-C10 a real 1Password account, so both are run by hand)', () => {
  ...
  async function sessionWithActiveWebTab(
    page: Page,
    env: E2EEnv,
    ws: 'ws-a' | 'ws-b' = 'ws-a'
  ): Promise<Locator> {
  ...
  test('BB-M02: the action icon sits at the right end of the address bar, divided from the nav controls', async ({
    ...
    const { app, page } = await launchSettled(env)
    try {
      await sessionWithActiveWebTab(page, env)
  ...
  test('BB-M03: clicking the action icon pops the extension popup, anchored under the icon', async ({
    ...
    try {
      ...
      const hostContentOnScreen = await hostContentBounds(app)
      const iconBox = await box(icon)
      const addressBar = await box(page.locator(BROWSER.addressBar))

      expect(popup.x).toBeLessThan(hostContentOnScreen.x + iconBox.x + iconBox.width)
      expect(popup.x + popup.width).toBeGreaterThan(hostContentOnScreen.x + iconBox.x)
      expect(popup.y).toBeGreaterThanOrEqual(
        hostContentOnScreen.y + addressBar.y + addressBar.height
      )
    } finally {
      await app.close().catch(() => {})
    }
  ...
  test('BB-M05: Settings → Extensions, between Notifications and About, lists the installed extension with its version', async ({
    env
  }) => {
    test.setTimeout(240_000)
    const probe = stageExtension(env, 'base')
  ...
  test('BB-M09: “Open Chrome Web Store” opens the session-independent, near-full-window store overlay', async ({
    page
  }) => {
    ...
    const NEAR_FULL_WINDOW_FRACTION = 0.8
    expect(overlay!.width).toBeGreaterThanOrEqual(viewport.width * NEAR_FULL_WINDOW_FRACTION)
    expect(overlay!.height).toBeGreaterThanOrEqual(viewport.height * NEAR_FULL_WINDOW_FRACTION)
    await expect
  ...
  test('BB-M10: a tab the extension opens lands active in the current session, with no unread dot', async ({
    ...
    try {
      ...
      await showWorkbench(page)
      await expect
        .poll(() => wbTabTitles(page), { timeout: 20_000 })
        .toEqual([PINNED_FILES_TAB_LABEL, 'M10A'])
    } finally {
      await app.close().catch(() => {})
      await server.close()
    }
  ...
  test('BB-M11: an installed extension survives a restart without the install seam', async ({
    env
  }) => {
    test.setTimeout(420_000)
    const probe = stageExtension(env, 'base')
  ...
  test('the `browser` alias is added only in chrome-extension: frames and extension service workers, so an ordinary web page in the browser partition gets no `browser` global', async ({
    env
  }) => {
    test.setTimeout(240_000)
    const probe = stageExtension(env, 'base')
    ...
    try {
      await browserSession(page, env)
      await openTabOn(page, server.page('/no-alias', titled('NO-ALIAS')))
      const guest = await guestByUrl(app, '/no-alias')
      await expect.poll(() => rootAttr(guest, PROBE.injected), { timeout: 30_000 }).toBeTruthy()
      const globals = await guest.evaluate(() => ({
        chrome: typeof (window as unknown as { chrome?: unknown }).chrome,
        browser: typeof (window as unknown as { browser?: unknown }).browser
      }))
      expect(globals).toEqual({ chrome: 'object', browser: 'undefined' })
    } finally {
      await app.close().catch(() => {})
      await server.close()
    }
  ...
  test('BB-C13: an extension page mixing the browser namespace (its popup) does not crash', async ({
    ...
    try {
      ...
      await expect
        .poll(async () => (await extensionPopups(app)).length, { timeout: 30_000 })
        .toBe(1)
  ...
  // PLATFORM§14
  test('BB-C17: the browser guest presents a consistent Google Chrome brand in userAgentData and the other real-Chrome tells, so Google sign-in does not read it as an embedder', async ({
    env
  }) => {
    test.setTimeout(240_000)
    const server = await startEchoServer()
