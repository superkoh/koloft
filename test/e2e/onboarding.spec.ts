...
// PR 1 — the three layers of help a first-time user meets:
//  A  the four welcome steps in the centre slot (T-OB-01…T-OB-07),
//  C  Settings ▸ Welcome, the same content on demand (T-OB-08),
//  C' "What's new" once per upgrade (T-OB-09, T-OB-10).
//
// The welcome takes the slot T-LIFE-13 ① used to own, so that case now seeds
// `onboardingSeen: true` and keeps pinning the panel a user who removed every
// workspace lands on; the first-run copy is asserted here instead.
//
// One test-only seam this spec drives: KOLOFT_TEST_CLAUDE_PROBE=missing — the "no claude
// on this Mac" branch of step 4, without touching PATH (the fixture's own fake `claude`
// lives there). C' rides on KOLOFT_UPDATE_FIXTURE alone: main runs the check for a
// fixture-driven run exactly as it does for a packaged one, and nothing here ever reaches
// the network.
//
// The folder picker is answered from KOLOFT_FILE_DIALOG_FILE (helpers/env.ts sets it for
// every launch): a native open panel in a background run takes focus and then hangs.

/** A never-run install: nothing pinned, the welcome not yet seen. */
...
/** The welcome's own step counter — the one observable that says which step is showing
 *  regardless of what that step renders. */
...
/** Walk the welcome from step 1 up to `step`, always on the primary button. Step 2 waits
 *  for the discovered folders first: its button is disabled until they land, and once they
 *  have, the click means "pin these". */
...
test('T-OB-01: a first run lands on welcome step 1', async ({ env }) => {
  ...
  try {
    ...
    // the titlebar's Add-workspace icon stays lit: it is the other way forward
    await expect(page.locator('.tb-ico[aria-label="Add workspace"]')).toHaveClass(/\bon\b/)
    // the sidebar's "No workspaces yet / press ⇧⌘O" hint would say step 2 twice
...
test('T-OB-02: step 2 lists the folders Claude Code was used in, and pins the checked ones', async ({
  env
}) => {
  test.setTimeout(120_000)
  firstRun(env)
  const now = Date.now()
  // ws-a: two transcripts, the newest of the two folders
  ...
  // a session inside a worktree: the folder to pin is the repo above it, never the copy
  const wt = path.join(env.home, 'repo', '.claude', 'worktrees', 'wt-1')
  fs.mkdirSync(wt, { recursive: true })
  seedJsonl(env, wt, { summary: 'Worktree one', mtime: now - 172_800_000 })
  // a folder that no longer exists — never created, so its cwd is gone
  ...
  try {
    ...
    // ws-a alone reaches the sidebar, and pinning it does not evict the welcome
...
test('T-OB-03: with no history behind it step 2 is a plain folder pick', async ({ env }) => {
  ...
  try {
    ...
    // the picker answers ws-a (helpers/env.ts's dialog queue), so the click IS the pin
...
test('T-OB-04: Skip hands the slot back to the plain panel, for good', async ({ env }) => {
  ...
  try {
    ...
    // the same slot, now the "a user who removed every workspace" panel of T-LIFE-13 ①
  ...
  try {
    ...
    await page.waitForLoadState('domcontentloaded')
    await expect(page.locator('.w-empty .big')).toHaveText('No workspace yet', { timeout: 20_000 })
    await expect(welcome(page)).toHaveCount(0)
  } finally {
...
test('T-OB-05: finishing the welcome starts the first session in the pinned folder', async ({
  ...
  try {
    ...
    // the probe finds the fixture's fake `claude` on PATH, so the step offers the launch
...
test('T-OB-06: with no supported CLI installed step 4 says how to get one and launches nothing', async ({
  ...
  try {
    ...
    await primary.click()
    // a re-probe, not a launch: the step stays where it is and no claude is spawned
...
test('T-OB-07: picking the balancing card ends in Settings ▸ Accounts — no launch, no setting changed', async ({
  ...
  try {
    ...
    // with no account registered yet there is nothing to sign a session in as, so the
    // step's primary is the accounts pane itself, not a launch
    ...
    // the card is a promise to show the pane, never a switch that turns balancing on
    await expect.poll(() => settingsOnDisk(env).onboardingSeen, { timeout: 20_000 }).toBe(true)
...
test('T-OB-08: Settings ▸ Welcome is the same help on demand, and can replay it', async ({
  ...
  try {
    ...
    await expect(page.locator('.settings-modal')).toBeVisible({ timeout: 20_000 })
    // the newcomer's pane is the first thing the nav offers
    ...
    // nothing is pinned, so "the next time no workspace is pinned" is right now
    await expect(welcome(page)).toBeVisible({ timeout: 20_000 })
    await expectStep(page, 1)
  } finally {
    await quitAndClose(app)
...
/** Point the release feed at a local file holding these releases. The versions are
 *  deliberately far below anything `app.getVersion()` can report (an unpackaged run reports
 *  ELECTRON's version), so the fixture can be written before launch — which C' needs, since
 *  it is asked at renderer start. */
...
/** Nothing opened. A launch whose decision RECORDS a version has that write as its
 *  observable; the ones that write nothing have none, so they wait out a beat instead. */
async function expectNoModal(page: Page, env: E2EEnv, recorded?: string): Promise<void> {
  ...
  if (recorded) {
    await expect.poll(() => settingsOnDisk(env).lastSeenVersion, { timeout: 20_000 }).toBe(recorded)
  } else {
...
test('T-OB-09: an upgrade shows what changed once, then remembers', async ({ env }) => {
  ...
  try {
    ...
    // release notes, not an update offer
    ...
    await expect.poll(() => settingsOnDisk(env).lastSeenVersion, { timeout: 20_000 }).toBe(version)
  } finally {
    await quitAndClose(app1)
  }

  // the feed still has both releases; what changed is that this version has been read
...
test('T-OB-10: a fresh install and an unreadable feed say nothing', async ({ env }) => {
  test.setTimeout(240_000)
  const fixture = withWhatsNew(env, [['0.1.2', 'the newer thing']])
  seedSettings(env, { onboardingSeen: true, lastSeenVersion: '' })

  // (a) first install: nothing to have missed, but the version is recorded so the NEXT
  // upgrade knows where to start
  const app1 = await launchApp(env)
  let version = ''
  try {
    const page = await app1.firstWindow()
  ...
  // (c) an upgrade whose feed cannot be read: say nothing AND remember nothing, so the
  // next launch gets another go at it
