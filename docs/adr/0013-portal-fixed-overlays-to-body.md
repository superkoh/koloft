...
**Decision**: every `position: fixed` overlay (the row context menu, the hints, the
account-usage card) renders into `document.body` through `createPortal`, even where no
transformed parent exists today.
